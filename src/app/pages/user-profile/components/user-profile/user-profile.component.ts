...
export class UserProfileComponent implements OnInit, OnDestroy {
  ...
  public isChangingAvatarLoading$: Observable<boolean> = this.store.select(selectLoading, {
    type: LoadingTypes.USER_AVATAR,
  });
  ...
  private initializeForm(userProfile: UserProfile): FormGroup<UserProfileInfoForm> {
    return new FormGroup({
      description: new FormControl(userProfile.info, [Validators.maxLength(500)]),
    });
  }
